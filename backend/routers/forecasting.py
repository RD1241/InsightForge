...
async def get_train_progress():
    """
    Polled by the frontend while a training run is in flight (POST /train runs in a
    thread-pool executor, so this GET is served concurrently on the same event loop).
    """
